...
TEST_CASE("Constructors") {
    ...
    SECTION("move") {
        ...
        SECTION("initializes moved_decimal.integerPart to \"131\" ")
            REQUIRE("131" == moved_decimal.getIntegerPart());

        SECTION("initializes moved_decimal.fractionalPart to \"013\"")
            REQUIRE("013" == moved_decimal.getFractionalPart());

        SECTION("initializes moved_decimal.precision to 3")
            REQUIRE(3 == moved_decimal.getPrecision());

        SECTION("initializes moved_decimal.isNegative to true")
            REQUIRE(true == moved_decimal.getIsNegative());

        SECTION("zeroing a decimal.integerPart")
            REQUIRE(decimal.getIntegerPart().empty());

        SECTION("zeroing a decimal.freactionalPart")
            REQUIRE(decimal.getFractionalPart().empty());

        SECTION("zeroing a decimal.precision")
            REQUIRE(decimal.getPrecision() == 0);

        SECTION("zeroing a decimal.isNegative")
            REQUIRE(decimal.getIsNegative() == false);
    }
}
...
TEST_CASE("Decimal Utility Functions") {
    SECTION("Round") {
...
TEST_CASE("Decimal Input/Output") {
